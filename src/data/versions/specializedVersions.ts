
import { Version } from '../versionInterface';

export const specializedVersions: Version[] = [
  {
    name: "V10",
    title: "Children's Picture Books",
    description: "Powered by GPT-4o with image generation capabilities. Specialized in creating illustrated children's picture books with engaging stories and vibrant visuals.",
    color: "bg-teal-800/30 border-teal-700/50",
    textColor: "text-slate-200",
    descriptionColor: "text-slate-300",
    accentColor: "text-teal-400",
    buttonColor: "bg-teal-600 hover:bg-teal-700",
    rating: 4.7,
    features: [
      "GPT-4o powered image generation",
      "Children's book illustrations",
      "Age-appropriate storytelling",
      "Character development for kids",
      "Educational content options",
      "Interactive visual elements",
      "Simple vocabulary adjustment",
      "Parent-friendly controls"
    ],
    url: "https://childrensbookmaker.lovable.app/?via=aiwebtools"
  },
  {
    name: "V11",
    title: "Coloring Book Maker",
    description: "Powered by GPT-4o with image generation. Creates custom coloring pages for kids and adults with personalized themes and educational content.",
    color: "bg-purple-800/30 border-purple-700/50",
    textColor: "text-slate-200",
    descriptionColor: "text-slate-300",
    accentColor: "text-purple-400",
    buttonColor: "bg-purple-600 hover:bg-purple-700",
    rating: 4.8,
    features: [
      "Custom coloring pages",
      "Personalized themes",
      "Age-appropriate designs",
      "Educational content",
      "Multiple difficulty levels",
      "Printable high-quality output",
      "GPT-4o powered image generation"
    ],
    url: "https://coloringbookmaker.lovable.app/?via=aiwebtools"
  },
  {
    name: "Cover Maker",
    title: "Book Cover Maker",
    description: "Powered by GPT-4o with image generation. Creates professional book covers for your stories with custom designs for both front and back covers.",
    color: "bg-blue-800/30 border-blue-700/50",
    textColor: "text-slate-200",
    descriptionColor: "text-slate-300",
    accentColor: "text-blue-400",
    buttonColor: "bg-blue-600 hover:bg-blue-700",
    rating: 4.9,
    features: [
      "Front and back cover designs",
      "Custom typography selection",
      "Professional cover layouts",
      "Genre-appropriate styling",
      "High-resolution output",
      "Marketing-ready designs",
      "GPT-4o powered image generation"
    ],
    url: "https://chatgpt.com/g/g-6812c1c50ed48191b6b1647d7d52c62b-graphic-cover-design-gpt"
  },
  {
    name: "Comic Book",
    title: "Comic Book Generator",
    description: "Powered by GPT-4o with image generation. Creates full comic books with panels, designs, and storylines in collaboration with the user.",
    color: "bg-green-800/30 border-green-700/50",
    textColor: "text-slate-200",
    descriptionColor: "text-slate-300",
    accentColor: "text-green-400",
    buttonColor: "bg-green-600 hover:bg-green-700",
    rating: 4.3,
    features: [
      "Comic book panel generation",
      "Character design assistance",
      "Story development tools",
      "Interactive storytelling",
      "Custom comic art styles",
      "Full comic book layouts",
      "GPT-4o powered image generation"
    ],
    url: "https://comicbookmakergpt.lovable.app/?via=aiwebtools"
  },
  {
    name: "Movie Script",
    title: "Movie Script Writer",
    description: "Powered by OpenAI's GPT technology. Creates full industry professional movie scripts with proper formatting and structure.",
    color: "bg-red-800/30 border-red-700/50",
    textColor: "text-slate-200",
    descriptionColor: "text-slate-300",
    accentColor: "text-red-400",
    buttonColor: "bg-red-600 hover:bg-red-700",
    rating: 4.9,
    features: [
      "Industry standard formatting",
      "Professional screenplay structure",
      "Character development",
      "Scene descriptions",
      "Dialogue writing",
      "Full-length feature scripts",
      "Powered by OpenAI's GPT"
    ],
    url: "https://moviescriptwritergpt.lovable.app/"
  }
];
